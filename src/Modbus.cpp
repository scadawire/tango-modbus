...
void Modbus::init_device()
{
	...
	if (cacheConfig.empty() == false)
	{
		...
		if (cacheSleep_sec < 1)
			cacheSleep_sec = 1;
		maxDeltaTh = (3 * 2) + cacheSleep_sec * 3 + (Tango::DevLong)(cacheDef.size() * tCPTimeout + 0.5);
...
void Modbus::get_device_property()
{
	...
	dev_prop.push_back(Tango::DbDatum("TCPConnectTimeout"));
	...
	if (dev_prop.size()>0)
	{
		...
		//	Try to initialize TCPConnectTimeout from class property
		cl_prop = ds_class->get_class_property(dev_prop[++i].name);
		if (cl_prop.is_empty()==false)	cl_prop  >>  tCPConnectTimeout;
		else {
			//	Try to initialize TCPConnectTimeout from default device value
			def_prop = ds_class->get_default_device_property(dev_prop[i].name);
			if (def_prop.is_empty()==false)	def_prop  >>  tCPConnectTimeout;
		}
		//	And try to extract TCPConnectTimeout value from database
		if (dev_prop[i].is_empty()==false)	dev_prop[i]  >>  tCPConnectTimeout;
...
Tango::DevVarShortArray *Modbus::read_holding_registers(const Tango::DevVarShortArray *argin)
{
	...
	int data_block = get_data_block("readholdingregisters",register_address,no_registers);
	...
	if (data_block == -1)
	{
	  ...
	  // Do as many readouts as required (120 registers max per frame)
	  do  {
	  ...
	  argout->length(registers.size());
	  for(size_t i=0;i<registers.size();i++)
	    (*argout)[i] = registers[i];

	} else {
          argout  = new Tango::DevVarShortArray();
	  get_cache_data(data_block,register_address,no_registers,argout);
	}
...
Tango::DevVarShortArray *Modbus::read_input_registers(const Tango::DevVarShortArray *argin)
{
	...
	int data_block = get_data_block("readinputregisters",register_address,no_registers);
...
Tango::DevVarShortArray *Modbus::read_multiple_coils_status(const Tango::DevVarShortArray *argin)
{
	...
	check_argin(argin,2,"Modbus::read_multiple_coils_status");
	...
	if (data_block == -1) {
	  ...
	  for (int seq=0, idx=0 ; seq < no_bytes ; seq++)
	  {
	    ...
	    for (int i=0 ; (i < 8)  && (idx < no_coils); i++)
	    {
          ...
          argout  = new Tango::DevVarShortArray();
	  get_cache_data(data_block,coil_address,no_coils,argout);
	}
 ...
 */
//--------------------------------------------------------
void Modbus::add_dynamic_commands()
{
	/*----- PROTECTED REGION ID(Modbus::add_dynamic_commands) ENABLED START -----*/
	
	//	Add your own code to create and add dynamic commands if any
	
	/*----- PROTECTED REGION END -----*/	//	Modbus::add_dynamic_commands
}
...
//---------------------------------------------------------------------------
// Check if the command result is in the data cache and return data block id
//---------------------------------------------------------------------------

int Modbus::get_data_block(const char *cmd,short adr,short nb_reg) {
  ...
  // Indentifie calling thread
  omni_thread *th = omni_thread::self();
  if (th != 0) {
    int th_id = th->id();
    if (th_id == thId)
      // ChaceThread is the caller
      return -1;
  } else {
    return -1;
  }
...
//------------------------------------------------------------
// Retrieve cahched data
//------------------------------------------------------------
void Modbus::get_cache_data(int data_block,short input_address,short no_inputs,Tango::DevVarShortArray *argout) {
